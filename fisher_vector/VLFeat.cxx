...
extern "C" {
short* ctypes_get_dense_sift(int width, int height, int step, int bin, int numScales, int scaleStep,
                          float* image, int image_size, int* num_sifts);
}
...
short* ctypes_get_dense_sift(int width, int height, int step, int bin, int numScales, int scaleStep,
  ...
  free (dSiftSet);
  short* shortResult = (short*) malloc(numDesc*(dims+3)*sizeof(short));
  ...
  if (shortResult != NULL) {  // loop throug and make unsigned dims and put in short to save memory
    int currLoc = 0;
    for (int i=0; i<numDesc; i++) {
      ...
      for (int x=0; x<dims; x++) {
        ...
        shortResult[currLoc++] = ((v < 255) ? v : 255);
      } // end for x
      shortResult[currLoc++] = (unsigned short) keypoints[i].x;
      shortResult[currLoc++] = (unsigned short) keypoints[i].y;
      if (keypoints[i].s != 0) {
        shortResult[currLoc++] = (unsigned short) keypoints[i].s;
      } else {
        shortResult[currLoc++] = 0;
      }
    } // end for i
  } else {
