...
def load_dataset(dataset_name):
    ...
    wrapper = s3_utils.S3Wrapper()
    if dataset_name == 'cifar-10':
        return np.load(io.BytesIO(wrapper.get(key="datasets/cifar.npz")))
...
def preprocess(train, test, min_divisor=1e-8, zca_bias=0.0001, return_weights=False):
    ...
    if (return_weights):
        return (train.reshape(origTrainShape).astype('float32'), test.reshape(origTestShape).astype('float32')), global_ZCA
    else:
        return (train.reshape(origTrainShape).astype('float32'), test.reshape(origTestShape).astype('float32'))
